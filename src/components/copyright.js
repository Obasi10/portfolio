...
const Copyright=({w, changeset})=>{

    return (
        <motion.footer className="align-center copyright">
                {(document.documentElement.clientWidth<=700 && changeset!==0) && (<span style={{color:"black"}}>&copy; 2023</span>)}{(w>700 || changeset===0) && (<span style={{color: "#eeeeee"}}>&copy; 2023</span>)}<span className="nav-item px-2" style={{color:"rgb(1, 32, 200)"}}>Designer: <motion.span variants={Variants} animate="designer"><Link to="/about" className="Designer" 
               >{(document.documentElement.clientWidth<=700 && changeset!==0) && (<span style={{color:"black"}}> Innocent obasi</span>)}{(w>700 || changeset===0) && (<span style={{color: "#eeeeee"}}> Innocent obasi</span>)}
                    </Link></motion.span></span>
                    {w>350 && (<><span className="px-2 nav-link border-0"><Link to="https://www.linkedin.com/in/innocent-obasi-72ab6022a" target="_blank">
